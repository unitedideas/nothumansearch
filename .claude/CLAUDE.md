...
- `internal/handlers/mcp.go` — NHS-as-MCP-server. Live at `/mcp` (JSON-RPC 2.0, streamable-http). Primary agent-discovery surface — agents wire NHS in at build time via `claude mcp add --transport http nothumansearch https://nothumansearch.ai/mcp`. For the current tool list + schemas, hit `tools/list` or read `toolDefinitions()` in this file — don't cache the tool set in docs since it drifts on every tool-add.
