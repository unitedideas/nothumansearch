...
Timestamp: `2026-05-31T13:09:17Z`
...
- A future executor must run `tools/geo-jobs-redacted-read.sh` successfully before any private score-fix mutation and may classify or clean up only `test_like pending` rows through the private admin workflow.
...
The repo-local proof, ledger append, and generated-work-item refresh were written in the worktree, but committing is blocked in this executor because Git cannot create `.git/index.lock`:
...
- Commit `harness/work_machine_50e939bf54d3db5e.md`.
- Clear assume-unchanged flags and commit `ops/ledgers/score-fix-internal-test-cleanup-2026-05-12.md` and `harness/generated-work-items.json`.
