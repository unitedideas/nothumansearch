...
- `harness/work_machine_0288ea9945bc8692.md` was required by the WorkItem but is not present in this checkout.
- Recent available closeout `harness/work_machine_51465b9dcbbab335.md` was read as the tracked successor context.
...
- Keep the external `real_candidate pending` rows untouched. Prior external follow-up proof in `ops/ledgers/score-fix-pending-followup-2026-05-12.md` still blocks another customer-visible score-fix email unless a future duplicate check and fresh public-action lock prove it is due.
