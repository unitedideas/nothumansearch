...
- External real-candidate pending rows stay untouched; prior follow-up proof still blocks another customer-visible score-fix email unless a future duplicate check and fresh public-action lock prove it is due.
...
- `GOCACHE=/private/tmp/nhs-gocache go test ./...`: failed in `internal/handlers` on `TestCheckRateLimitResponseAdvertisesPaidAPIHandoff`; this run made no code changes.
