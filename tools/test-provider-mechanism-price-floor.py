...
BASELINE_PATH = pathlib.Path(__file__).parents[1] / "docs/experiments/nhs-cost-baseline-2026-08-11.json"
...
class ProviderMechanismPriceFloorTest(unittest.TestCase):
    def test_baseline_is_exact_and_reconciled(self):
        ...
        self.assertEqual(baseline["observed_cost_cents"], 1157)
        self.assertEqual(sum(item["observed_cost_cents"] for item in baseline["applications"]), 1157)
    ...
    def test_equal_allocation_reconciles_odd_cent(self):
        allocation = MODULE.allocate_equal_cost(1157)
        self.assertEqual(allocation, {"accepted": 386, "activated": 386, "converted": 385})
        self.assertEqual(sum(allocation.values()), 1157)
